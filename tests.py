...
def test_select_all(session):
    result = tasks.select_all_tasks(session)
...
def test_update_status(session, connection):
    tasks.update_status(session, connection, "complete", 1)
...
def test_update_description(session, connection):
    tasks.update_description(session, connection, "new_description", 3)
    session.execute("SELECT * FROM tasks")
    result = session.fetchall()
...
def test_select_one_task(session):
    result = tasks.select_one_task(session, 2)
...
def test_delete_task(session, connection):
    tasks.delete_task(session, connection, 3)
    session.execute("SELECT * FROM tasks")
    result = session.fetchall()
...
def test_insert_task(session, connection):
    tasks.insert_task(session, connection, "Example 3")
    session.execute("SELECT * FROM tasks")
    result = session.fetchall()
